...
class Inventory:
    ...
    def _select_zima_leto(self, s_param, w_param):
        cur_time = datetime.datetime.now()
        if 5 <= cur_time.month <= 10:
            return True if random.randint(1, 100) < s_param else False
        return True if random.randint(1, 100) < w_param else False
...
def get_prihod():
    prihod_inv = [Sanki, Snowboard, Luji, Palki]
    for i in range(1, 100):
        cls = random.choice([Sanki, Snowboard, Luji, Palki])
        prihod_inv.append(cls(random.randint(100, 1000)))
    return prihod_inv
